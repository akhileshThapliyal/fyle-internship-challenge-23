...
export class HttpCacheService implements HttpCache {
	...
	constructor() {
		console.debug(`HttpCacheService constructed for ${Object.keys(this.cachableRoutes).join(',\n')}`);
	}
	...
	shouldCache(urlWithParams: string) {
		...
		Object.keys(this.cachableRoutes).forEach((pattern) => {
			...
			console.log("Pattern => ", pattern, " urlWithParams => ", urlWithParams);
	...
	shouldCacheToSessionStorage(urlWithParams: string) {
		...
		Object.keys(this.cachableRoutes).forEach((pattern) => {
			...
			const routeMatch = route.match(urlWithParams);
			if (routeMatch 
					&& this.cachableRoutes[pattern] === true) {
				shouldCache = !!routeMatch;
			}
		});
		return shouldCache;
